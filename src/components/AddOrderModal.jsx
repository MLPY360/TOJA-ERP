...
export default function AddOrderModal({ isOpen, onClose }) {
  const { products, addOrder, language, blacklist } = useStore();
  ...
  const handleSubmit = (e) => {
    ...
    for (const item of validItems) {
      ...
      if (product) {
        ...
        if (item.qty > stock) {
          alert(`${product.name} (${item.size}): ${t.lowStock} - Max: ${stock} ${t.leftInStock}`);
          return;
        }
      }
    }

    // Blacklist check confirmation
    if (blacklistedMatch) {
      const proceed = window.confirm(
        `${t.blacklistWarningTitle}\n${blacklistedMatch.reason ? `${t.blacklistReason}: ${blacklistedMatch.reason}\n` : ''}${t.blacklistRecommendation}\n\nDo you want to create this order anyway?`
      );
      if (!proceed) return;
    }
      ...
      <div dir={language === 'ar' ? 'rtl' : 'ltr'} className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        ...
        >
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-100 bg-slate-50 shrink-0">
            <h2 className="text-lg sm:text-xl font-extrabold text-[#181E1C]">{t.createNewOrder}</h2>
                        ...
                        <div className="flex items-end gap-3 w-full sm:w-auto">
                          ...
                          <div className="flex-1 sm:w-20">
                            ...
                            <input
                              type="number"
                              min="1"
                              max={selectedStock && selectedStock > 0 ? selectedStock : undefined}
                              value={item.qty}
                              onChange={e => updateItem(index, 'qty', e.target.value)}
                              className="w-full h-10 px-3 rounded-lg border border-slate-200 bg-white text-sm outline-none focus:border-[#597867] text-start"
                            />
                          </div>
